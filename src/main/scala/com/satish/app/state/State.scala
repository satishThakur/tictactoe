...
/**
 * What State represent is a State transformation function. It takes the current state,
 * produces the value A and trnasition to state to a new state.
 * @tparam S - State
 * @tparam A - Value
 */
...
object State:
  extension[S,A] (other: State[S,A])
    /**
    * run finally runs the STF to produce value and modified state.
     * This is mostly done on the edge of the application.
    */
    def run(s: S): (A,S) = other(s)

    /**
     * given a STF and a function which maps A to B -
     * Create new STF which produces B.
     */
    ...
    /**
     * given another STF who depends on value A - produce it.
     * Again state is threaded via both STFs.
     *
     */
