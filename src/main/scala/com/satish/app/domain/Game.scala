...
case class Game(board: Board, current: Player, players: (Player, Player)):
  ...
  def move(cell: Cell): Game =
    val stf = Game.transition(cell, current.piece)
    stf.run(this)(1)
...
object Game:
  ...
  private def singleMove(cell: Cell, piece: Piece)(g: Game): Game =
  ...
  private def transition(cell: Cell, piece: Piece): State[Game, Status] = for {
