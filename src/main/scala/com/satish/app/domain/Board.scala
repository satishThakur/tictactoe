...
case class Board(state: Map[Cell, Piece]):
  ...
  /** Places given piece at given cell and returns new board.
    ...
    *   Cell
    * @param piece
    *   Piece to be placed at given position.
    * @return
    *   New board with updated state.
    */
  ...
  def prettyPrint: String =
    ...
    val cellValues: List[String] = cells.map(c =>
      pieceAt(c)
        .map({
          case Piece.O => "O"
          case Piece.X => "X"
...
object Board:
  ...
  def apply(cells: (Cell, Piece)*): Board =
    cells.foldLeft(empty) { case (board, (cell, piece)) =>
      board.placePiece(cell, piece)
    }
